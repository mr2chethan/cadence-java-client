...
public class ReplayWorkflowActivityImplHeartbeatTest {
  ...
  @Test
  public void testUnknownPropertiesOfHeartbeatDetailAreIgnoredSilently() throws Exception {
    Class<?> detailClass = heartbeatDetailClass();
    byte[] details =
        ("{\"replayResult\":{\"succeeded\":3,\"skipped\":1,\"failed\":2,\"heartbeatTestExtra\":0},"
                + "\"replayExecutionIndex\":7,\"heartbeatTestNext\":\"x\"}")
            .getBytes(StandardCharsets.UTF_8);
    List<Object> decoded = new ArrayList<>();

    List<ILoggingEvent> logged =
        compatibilityLogOf(
            () -> decoded.add(converter.fromData(details, detailClass, detailClass)));

    assertEquals(Collections.emptyList(), logged);
    assertProgress(decoded.get(0));
  }
  ...
  public void testActivityParametersWrittenByShadowingWorkflow() {
    byte[] input =
        ("{\"domain\":\"samples\",\"executions\":[{\"workflowId\":\"w-1\",\"runId\":\"r-1\","
                + "\"shadowingTestExtra\":1}],\"shadowingTestNext\":{\"a\":[]}}")
            .getBytes(StandardCharsets.UTF_8);
    List<Object[]> decoded = new ArrayList<>();

    List<ILoggingEvent> logged =
        compatibilityLogOf(
            () -> decoded.add(converter.fromDataArray(input, ReplayWorkflowActivityParams.class)));

    assertEquals(Collections.emptyList(), logged);
    ReplayWorkflowActivityParams params = (ReplayWorkflowActivityParams) decoded.get(0)[0];
